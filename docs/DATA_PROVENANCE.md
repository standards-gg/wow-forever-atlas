...
| **WoW: Forever client (local install)** | Yes | Yes | Yes — project owner's explicit decision | Yes — project owner's explicit decision | Attribution to Blizzard as source, not a license grant | Yes, per owner decision | Terrain/minimap textures extracted directly via `importers/wow-client` and committed to this public repo. This differs from the DB2/data-only sources above in kind (rendered art, not facts/positions) and in posture: the project owner explicitly weighed this and chose to proceed, citing the same long-standing community norm every major WoW fan site (Wowhead, wow.export, Wowpedia) already operates under. Revisit if that norm or the project's context changes (e.g. a commercial launch). |
