...
/**
 * Pure types and query functions, split out from world-data.ts's
 * `loadWorldGeography` (which touches `node:fs/promises`) for the same
 * reason atlas-data-core.ts is split from atlas-data.ts — client
 * components (AtlasMap, via lib/world-frame.ts) need this logic without
 * pulling a Node-only module into the browser bundle.
 */
